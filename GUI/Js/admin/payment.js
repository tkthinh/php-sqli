...
async function checkLogin_payment() {
  try {
...
async function getDataPermission_payment(codePermission) {
  try {
...
function checkUpPermission_payment(dataPermissionDetail, functionPoint) {
  if (functionPoint == "") {
    return false;
  }

  for (let item of dataPermissionDetail) {
    if (item.functionCode == "payment") {
...
async function delete_PaymentsById(code) {
  ...
  let codePermission = await checkLogin_payment();
  // lấy mảng chi tiết phân quyền dựa theo mã phân quyền của người đăng nhập
  let data = await getDataPermission_payment(codePermission);
  // lấy thông tin có được phép làm chức ăng đó không
  let check = checkUpPermission_payment(data.permissionDetail, "delete");
...
// thêm một đối tương
async function add_PaymentObj(event) {
  // lấy thông tin mã  codePermission của người đăng nhập
  let codePermission = await checkLogin_payment();
  // lấy mảng chi tiết phân quyền dựa theo mã phân quyền của người đăng nhập
  let data = await getDataPermission_payment(codePermission);
  // lấy thông tin có được phép làm chức ăng đó không
  let check = checkUpPermission_payment(data.permissionDetail, "add");

  if (check == true) {
    window.location.href = "../../../GUI/view/admin/add-payment.php";
  } else {
...
async function update_PaymentsObj(
  ...
  let data = await getDataPermission_payment(codePermission);
  // lấy thông tin có được phép làm chức ăng đó không
  let check = checkUpPermission_payment(data.permissionDetail, "update");
